  ...
  return [
    {
      ...
      description: [
        ...
        "waitSeconds controls ONLY how long this tool waits for the user (default 60, integer 0–300): positive values pause this turn until answered, timed out, dismissed, or cancelled; 0 returns awaiting_user immediately so you can continue independent work. waitSeconds is a fixed deadline measured from when the question is shown: user interaction never extends it, and an ended wait never restarts. This tool returns only request metadata and status, never answer content. All answers arrive as a separate user message linked to the original question: an in-time answer follows the tool result before you continue; asynchronous or late answers arrive later. Read that user message for the actual answers; do not look for answer.text/answer.parts in this tool result or infer answers from status alone.",
        "The panel has a separate 60-second inactivity auto-collapse timer, reset by user interaction; waitSeconds=10 does NOT close the panel after 10 seconds. Neither countdown is shown upfront: the panel shows its final 10 seconds; the model wait shows only its final min(10, waitSeconds/2) seconds. Interaction resets only the panel timer; it does not extend the model wait. Users can reopen unanswered questions from the original tool row, even after the turn finishes; reopening does not restart an ended model wait.",
      ...
      inputSchema: {
        type: "object",
        properties: {
          waitSeconds: {type: "integer", minimum: 0, maximum: 300, default: 60, description: "LLM wait only, NOT panel lifetime. 0 is asynchronous; 1–300 waits for an answer. Fixed deadline from when the question appears; interaction does not extend it. The panel separately collapses after 60 seconds of inactivity."},
