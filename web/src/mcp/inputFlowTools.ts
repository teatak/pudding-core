  ...
  return [
    {
      ...
      description: [
        ...
        "waitSeconds controls ONLY how long this tool waits for the user (default 60, integer 0–300): positive values pause this turn until answered, timed out, dismissed, or cancelled; 0 returns awaiting_user immediately so you can continue independent work. User interactions renew an unexpired wait by waitSeconds; they never restart a wait that already ended. This tool returns only request metadata and status, never answer content. All answers arrive as a separate user message linked to the original question: an in-time answer follows the tool result before you continue; asynchronous or late answers arrive later. Read that user message for the actual answers; do not look for answer.text/answer.parts in this tool result or infer answers from status alone.",
