      ...
      <div className="border-t px-1 py-1">
        {visible.map((change) => (
          ...
          >
            ...
            <code className="relative z-[1] min-w-0 flex-1 truncate font-mono text-[11px]">{turnFileChangeLabel(change, changes)}</code>
            {change.additions > 0 ? <span className="relative z-[1] shrink-0 text-success">+{change.additions}</span> : null}
            {change.deletions > 0 ? <span className="relative z-[1] shrink-0 text-destructive">−{change.deletions}</span> : null}
          </button>
        ))}
      </div>
      {collapsible ? (
