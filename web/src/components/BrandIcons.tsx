    ...
    >
      <defs>
        {/* lobehub 风格的 4 层叠加多色实现 — 比 4-radial-gradient+mask 简洁:
            ...
            渐变坐标用 userSpaceOnUse,跟着 path 一起被 transform(translate+scale 0.6)
            缩放到 sparkle 实际位置,数值不需要重算。 */}
      ...
      {/* 白底 app icon — 跟 OpenAI/Claude/MiMo 三家保持一致的"实色底"风格,
          Gemini sparkle 在白底上呈现 Google 官方多色视觉 */}
      <rect width="23" height="23" x="0.5" y="0.5" rx="4" fill="#ffffff" stroke="var(--border)" />
      {/* 4 层叠加 sparkle:translate+scale 缩到中心 60%(4.8..19.2)留 padding */}
...
export function OpenAIIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      aria-hidden
    >
      {/* OpenAI 经典黑底白 logo(参考 ChatGPT app icon 视觉)。
          底色用 #0D0D0D 而非纯黑,在 dark mode 深色背景上仍能微微显出方块边界。 */}
...
export function ClaudeIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      aria-hidden
    >
      {/* Anthropic Claude 官方 coral 橙底 + 白色 swirl/burst logo */}
...
export function MimoIcon({ className }: { className?: string }) {
  // MiMo 没有官方 logo SVG(Xiaomi MiMo GitHub repo 不带 brand asset),
  // 用小米橙 #FF6900 底 + 白色 "Mi" 字样作 fallback。
  // 字号 13、weight 700,在 24x24 内居中视觉舒适(跟其它三家 logo 占比相近)。
  return (
    <svg
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
...
export function DeepSeekIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      aria-hidden
    >
      {/* DeepSeek 官方品牌蓝 #4D6BFE 底 + 白色鲸鱼/海豚剪影 */}
...
export function QwenIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      aria-hidden
    >
      {/* 阿里通义千问 Qwen 官方紫 #615CED 底 + 白色 hexagonal Q 标志 */}
      <rect width="24" height="24" rx="4" fill="#615CED" />
      <g transform="translate(4.8 4.8) scale(0.6)">
        <path
...
export function GrokIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      aria-hidden
    >
      {/* xAI Grok 用 X 字 brand mark(xAI 官方就是 X 品牌)。黑底白 X,跟
          OpenAI 黑底白 logo 同款"app icon 风格"。 */}
...
export function OpenRouterIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      aria-hidden
    >
      {/* OpenRouter 官方 router/proxy 标志(simple-icons 收录)。黑底白 logo,
          表达"流量在多个 provider 之间路由分发"。 */}
      <rect width="24" height="24" rx="4" fill="#000000" />
      <g transform="translate(4.8 4.8) scale(0.6)">
        <path
...
export function OllamaIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      aria-hidden
    >
      {/* Ollama 标志性 monochrome llama 剪影:白底 + 黑 logo,light/dark 模式
          下都保持品牌识别度(Ollama 官方就是黑/白二值,跟 OpenAI 反过来配比) */}
      <rect width="23" height="23" x="0.5" y="0.5" rx="4" fill="#ffffff" stroke="var(--border)" />
      <g transform="translate(4.8 4.8) scale(0.6)">
        <path
