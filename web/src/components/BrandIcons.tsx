// 品牌 logo SVG 集合。
// BrandIcon 统一负责背景和裁切;品牌 SVG 占满容器,logo 留白由品牌 mark 自己定义。
//
// 数据来源:simple-icons.org (gemini / openai / claude) + BuzzHive favicon + Xiaomi logo (mimo)。
// 各厂商品牌 / 商标 © 各自所有,这里只用于 UI 标识用途,不做商品化。
//
// app icon 设计原则:
//   1. 外层 span 控制背景、形状和尺寸
//   2. SVG 元素必须铺满外层;品牌自身的留白放在 SVG viewBox / path 定义里
//   3. 大多数 mark 使用 currentColor;Gemini 保留原 SVG 配色

import type { CSSProperties } from "react";
import { useId } from "react";

import { IdentityIcon, type IdentityIconRadius, type IdentityIconShape, type IdentityIconSize } from "@/components/IdentityIcon";
...
// 名称 → 品牌图标映射:profile 名或 preset id 命中即显示,未命中显示首字母
export function BrandIcon({
  ...
  if (!knownBrandName) {
    return (
      <IdentityIcon
        className={cn("border border-border/70 bg-transparent text-muted-foreground shadow-none", className)}
        radius={radius}
        shape={shape}
        size={size}
      >
        <span className={cn("grid size-full place-items-center font-medium leading-none", brandInitialTextClass(size), iconClassName)}>
          {brandInitial(name)}
        </span>
      </IdentityIcon>
    );
  ...
  const normalizedName = knownBrandName;
  ...
  return (
    <IdentityIcon
      className={cn(
        isOpenRouter
          ? "bg-white text-[#6834EE] shadow-[inset_0_0_0_1px_var(--border)] dark:bg-[#02090B] dark:text-[#C8FF00] dark:shadow-none"
          : lightBackground
            ? "bg-white text-black shadow-[inset_0_0_0_1px_var(--border)]"
            : "text-white",
        className,
      )}
      radius={radius}
...
function brandInitialTextClass(size: IdentityIconSize) {
  switch (size) {
    case "xs":
      return "text-[9px]";
    case "sm":
