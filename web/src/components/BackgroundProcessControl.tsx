  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button
          aria-label={label}
          className="h-8 gap-1 rounded-full px-2 text-xs font-normal text-muted-foreground hover:text-foreground"
          size="sm"
          type="button"
          variant="ghost"
        >
          ...
          {recent.length > 0 ? (
            <Collapsible open={recentOpen} onOpenChange={setRecentOpen}>
              <CollapsibleTrigger asChild>
                <button
                  className={cn(
                    "flex h-8 w-full items-center gap-1.5 rounded-md px-2 text-xs text-muted-foreground hover:text-foreground",
                    appPopoverItemStateClassName,
                  )}
                  type="button"
                >
  ...
  return (
    <div
      className={cn(
        "rounded-md",
        appPopoverControlItemStateClassName,
        expanded && appPopoverSelectedItemStateClassName,
      )}
    >
