...
export function BrowserOptionsMenu({
  ...
  return (
    <DropdownMenu open={menuOpen} onOpenChange={(open) => {
      setMenuOpen(open);
      if (open) void loadCredentialState().catch(() => undefined);
    }}>
          ...
          <>
            <div className="space-y-3 px-2 py-2">
              <div className="space-y-1">
                ...
                <div className="truncate text-xs text-muted-foreground">{originLabel(prompt.origin)}</div>
                <div className="truncate text-sm">{prompt.username || originLabel(prompt.origin)}</div>
              </div>
