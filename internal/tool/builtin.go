...
func BuiltinDefinitions() []provider.ToolDef {
	return []provider.ToolDef{
		...
		{
			Name:        FileRead,
			Description: "Read one small UTF-8 text file from a Pudding-managed file area or an authorized project directory. content is unnumbered; direct line counting is acceptable for short complete reads, while line-based patches for long, truncated, or unfamiliar files should use builtin_file_search or builtin_file_slice. This tool is text-only; use builtin_media_read for supported images or audio. For large text files use builtin_file_slice or builtin_file_search first.",
		...
		{
			Name:        FileSlice,
			Description: "Read a focused line slice from a UTF-8 text file. numberedContent includes the exact one-based source line numbers required by builtin_file_patch; copy source text without its displayed number prefix into old_lines. Supports origin=start for line ranges and origin=end for tail-style reads, with optional reverse order.",
		...
		{
			Name:        FilePatch,
			Description: "Apply one atomic multi-file text patch to authorized project files. Before action=edit, prefer start_line from builtin_file_search or builtin_file_slice.numberedContent for long, truncated, or unfamiliar files; counting lines from a short complete builtin_file_read result is acceptable. Each hunk uses exact old_lines from the same original file snapshot, without displayed line-number prefixes; all hunks use pre-patch line numbers and must not overlap. On a line mismatch, read a fresh numbered slice and rebuild the patch instead of guessing or fuzzy matching. Every file is validated before writing, so any failure leaves the worktree unchanged.",
		...
		{
			Name:        ComputerAct,
			Description: "Perform one or more approved Computer Use actions against the current state of an explicit app window. Every call uses the actions array: use one item for a single action and 2 to 32 items only when all targets are already known and no intermediate state must be inspected. Actions run in order, each semantic target is resolved against the live Accessibility tree, and execution stops at the first failure without rollback or retry. Semantic types press, set_value, select, and submit use elementID and may run in the background. Pointer actions click, drag, and scroll require the target app to remain foreground and use normalized current-window coordinates: top-left is 0,0 and bottom-right approaches 1,1. Do not batch pointer actions when a prior action may move the window or target. Click supports one left or right click, or a left double-click. Drag uses the left button. Scroll uses pixel deltas where positive deltaY scrolls down and positive deltaX scrolls right. Never guess coordinates. Pointer success confirms input delivery only; inspect current state before claiming a visual effect when Accessibility cannot expose it. Use select for selectable rows or items. Use submit only when the observed element exposes it on a focused, enabled, editable single-line text control; it invokes AXConfirm when available, otherwise sends exactly one Return key event directly to that app process with no modifiers or repeat. Actions do not automatically observe afterward. Reuse known stable element IDs and call builtin_computer_observe only when a target is unknown, a UI change must be inspected, or the result is uncertain. For computer_app_not_foreground, call builtin_computer_use_app with foreground=true. For outcome=unknown, never repeat the action and inspect current state. If the window is no longer found, reacquire windows with builtin_computer_use_app instead of observing the stale window ID.",
			InputSchema: json.RawMessage(`{"type":"object","properties":{"appID":{"type":"string","minLength":1,"maxLength":512},"windowID":{"type":"integer","minimum":1},"actions":{"type":"array","minItems":1,"maxItems":32,"description":"Ordered actions. One item performs a single action; multiple items form one uninterrupted batch.","items":{"type":"object","properties":{"type":{"type":"string","enum":["press","set_value","select","submit","click","drag","scroll"]},"elementID":{"type":"string","minLength":1,"description":"Required for press, set_value, select, and submit; omit for pointer actions."},"value":{"type":"string","maxLength":20000,"description":"Required only for set_value."},"x":{"type":"number","minimum":0,"exclusiveMaximum":1,"description":"Required for pointer actions: normalized x within the current window."},"y":{"type":"number","minimum":0,"exclusiveMaximum":1,"description":"Required for pointer actions: normalized y within the current window."},"toX":{"type":"number","minimum":0,"exclusiveMaximum":1,"description":"Required only for drag: normalized destination x."},"toY":{"type":"number","minimum":0,"exclusiveMaximum":1,"description":"Required only for drag: normalized destination y."},"button":{"type":"string","enum":["left","right"],"description":"Click button; defaults to left."},"clickCount":{"type":"integer","minimum":1,"maximum":2,"description":"Click count; defaults to 1. Two is allowed only for the left button."},"deltaX":{"type":"integer","minimum":-5000,"maximum":5000,"description":"Scroll pixels; positive moves right. Defaults to 0."},"deltaY":{"type":"integer","minimum":-5000,"maximum":5000,"description":"Scroll pixels; positive moves down. Defaults to 0."}},"required":["type"],"additionalProperties":false}}},"required":["appID","windowID","actions"],"additionalProperties":false}`),
			Capability:  store.ModeWork,
		},
	}
}
