...
func builtinRunnerDefinitions() []provider.ToolDef {
	return []provider.ToolDef{
		...
		},
		{
			...
			InputSchema: json.RawMessage(`{"type":"object","properties":{"scope":{"type":"string","enum":["skill","temp","project"],"description":"Target writable file area. Use skill for global user Skills and project for authorized local project directories."},"path":{"type":"string","minLength":1,"description":"Relative file path inside a managed area, or an absolute/relative path inside authorized project directories."},"content":{"type":"string","description":"Complete replacement content, including any desired trailing newline. An explicit empty string creates or clears an empty file; do not omit this field or pass null."}},"required":["scope","path","content"],"additionalProperties":false}`),
			Capability:  store.ModeCode,
		},
		{
			Name:        FileDelete,
