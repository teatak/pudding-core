...
func BuiltinDefinitions() []provider.ToolDef {
	return []provider.ToolDef{
		...
		{
			Name:        ComputerAct,
			Description: "Perform an approved Computer Use action against the current state of an explicit app window. Semantic actions press, set_value, select, and submit resolve a deterministic elementID against the live Accessibility tree and may run in the background. Use action_sequence for 2 to 32 semantic actions when all target element IDs are already known and no intermediate state must be inspected; each step is resolved live and the sequence stops at the first failure without rollback or retry. Pointer actions cannot be sequenced. Pointer actions click, drag, and scroll require the target app to remain foreground and use normalized current-window coordinates: top-left is 0,0 and bottom-right approaches 1,1. Click supports one left or right click, or a left double-click. Drag uses the left button. Scroll uses pixel deltas where positive deltaY scrolls down and positive deltaX scrolls right. Never guess coordinates. Pointer success confirms input delivery only; inspect current state before claiming a visual effect when Accessibility cannot expose it. Use select for selectable rows or items. Use submit only when the observed element exposes it on a focused, enabled, editable single-line text control; it invokes AXConfirm when available, otherwise sends exactly one Return key event directly to that app process with no modifiers or repeat. Actions are never retried automatically and do not automatically observe afterward. Reuse known stable element IDs and call builtin_computer_observe only when a target is unknown, a UI change must be inspected, or the result is uncertain. For computer_app_not_foreground, call builtin_computer_use_app with foreground=true. For outcome=unknown, never repeat the action and inspect current state. If the window is no longer found, reacquire windows with builtin_computer_use_app instead of observing the stale window ID.",
			InputSchema: json.RawMessage(`{"type":"object","properties":{"appID":{"type":"string","minLength":1,"maxLength":512},"windowID":{"type":"integer","minimum":1},"elementID":{"type":"string","minLength":1,"description":"Required for press, set_value, select, and submit; omit for pointer actions and action_sequence."},"action":{"type":"string","enum":["press","set_value","select","submit","click","drag","scroll","action_sequence"]},"value":{"type":"string","maxLength":20000,"description":"Required only for a single set_value action."},"actions":{"type":"array","minItems":2,"maxItems":32,"description":"Required only for action_sequence. Ordered semantic actions resolved against the live window.","items":{"type":"object","properties":{"elementID":{"type":"string","minLength":1},"action":{"type":"string","enum":["press","set_value","select","submit"]},"value":{"type":"string","maxLength":20000,"description":"Required only for set_value."}},"required":["elementID","action"],"additionalProperties":false}},"x":{"type":"number","minimum":0,"exclusiveMaximum":1,"description":"Required for pointer actions: normalized x within the current window; 0 is left and 1 is the excluded right edge."},"y":{"type":"number","minimum":0,"exclusiveMaximum":1,"description":"Required for pointer actions: normalized y within the current window; 0 is top and 1 is the excluded bottom edge."},"toX":{"type":"number","minimum":0,"exclusiveMaximum":1,"description":"Required only for drag: normalized destination x."},"toY":{"type":"number","minimum":0,"exclusiveMaximum":1,"description":"Required only for drag: normalized destination y."},"button":{"type":"string","enum":["left","right"],"description":"Click button; defaults to left."},"clickCount":{"type":"integer","minimum":1,"maximum":2,"description":"Click count; defaults to 1. Two is allowed only for the left button."},"deltaX":{"type":"integer","minimum":-5000,"maximum":5000,"description":"Scroll pixels; positive moves right. Defaults to 0."},"deltaY":{"type":"integer","minimum":-5000,"maximum":5000,"description":"Scroll pixels; positive moves down. Defaults to 0."}},"required":["appID","windowID","action"],"additionalProperties":false}`),
			Capability:  store.ModeWork,
		},
	}
}
