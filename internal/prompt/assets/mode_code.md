...
- Use Project Files when it is listed in Available Apps and structured file operations are useful. After loading it, use `builtin_file_read` for focused whole-file reads and one `builtin_file_patch` call per logical multi-file change. If it is absent, use focused CLI operations instead of trying to load it.
