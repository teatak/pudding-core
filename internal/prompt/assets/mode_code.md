...
- Use Source Control for staging, unstaging, and commits when it is listed in Available Apps because its approval and drift checks are stronger. Otherwise use the Git CLI subject to the current approval policy. Read-only Git operations should normally use the CLI.
- When modifying project files and Project Files is listed in Available Apps, load it and use `builtin_file_read` for focused whole-file reads plus one `builtin_file_patch` call per logical multi-file change. Only structured project-file tools contribute to the turn file diff; command output and command-side filesystem effects remain visible through Git status/diff instead. If Project Files is absent, use focused CLI operations without claiming those effects as turn-owned changes.
