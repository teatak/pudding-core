...
func (s *Store) ListProjectAppBindings(ctx context.Context, projectID string) ([]*store.ProjectAppBinding, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, err := s.getProjectDB(ctx, projectID); err != nil {
		return nil, err
	}
	rows, err := s.db.QueryContext(ctx, `SELECT id,project_id,app_id,connection_id,resource_type,resource_id,resource_name,metadata,is_primary,created_at,updated_at FROM project_app_bindings WHERE project_id=? ORDER BY is_primary DESC,created_at ASC`, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		binding, err := scanProjectAppBinding(rows)
		if err != nil {
			return nil, err
		}
...
func (s *Store) PutProjectAppBinding(ctx context.Context, binding *store.ProjectAppBinding) error {
	if err := store.NormalizeProjectAppBinding(binding); err != nil {
		return err
	}
	return s.tx(ctx, func(tx *sql.Tx) error {
		if _, err := getProjectTx(ctx, tx, binding.ProjectID); err != nil {
			return err
		}
		now := time.Now()
		created := now
		...
		err := tx.QueryRowContext(ctx, `SELECT project_id,created_at FROM project_app_bindings WHERE id=?`, binding.ID).Scan(&existingProjectID, &existingCreated)
		if err == nil {
			if existingProjectID != binding.ProjectID {
				return store.ErrInvalidProjectAppBinding
			}
			created = timeFromMS(existingCreated)
		} else if !errors.Is(err, sql.ErrNoRows) {
			return err
		}
		if binding.Primary {
			if _, err := tx.ExecContext(ctx, `UPDATE project_app_bindings SET is_primary=0,updated_at=? WHERE project_id=? AND app_id=? AND id<>?`, unixMS(now), binding.ProjectID, binding.AppID, binding.ID); err != nil {
				return err
			}
		}
		metadata, err := json.Marshal(binding.Metadata)
		if err != nil {
			return err
		}
		_, err = tx.ExecContext(ctx, `INSERT INTO project_app_bindings(id,project_id,app_id,connection_id,resource_type,resource_id,resource_name,metadata,is_primary,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET app_id=excluded.app_id,connection_id=excluded.connection_id,resource_type=excluded.resource_type,resource_id=excluded.resource_id,resource_name=excluded.resource_name,metadata=excluded.metadata,is_primary=excluded.is_primary,updated_at=excluded.updated_at`, binding.ID, binding.ProjectID, binding.AppID, binding.ConnectionID, binding.ResourceType, binding.ResourceID, binding.ResourceName, string(metadata), boolInt(binding.Primary), unixMS(created), unixMS(now))
		if err != nil {
			return err
		}
		binding.CreatedAt, binding.UpdatedAt = created, now
		return nil
	})
}

func (s *Store) DeleteProjectAppBinding(ctx context.Context, projectID, bindingID string) error {
	return s.tx(ctx, func(tx *sql.Tx) error {
		result, err := tx.ExecContext(ctx, `DELETE FROM project_app_bindings WHERE project_id=? AND id=?`, projectID, bindingID)
		if err != nil {
			return err
		}
...
func (s *Store) DeleteProjectAppBindingsByConnection(ctx context.Context, connectionID string) error {
	return s.tx(ctx, func(tx *sql.Tx) error {
		type projectApp struct{ projectID, appID string }
		rows, err := tx.QueryContext(ctx, `SELECT DISTINCT project_id,app_id FROM project_app_bindings WHERE connection_id=? AND is_primary=1`, strings.TrimSpace(connectionID))
		if err != nil {
			return err
		}
		var affected []projectApp
		for rows.Next() {
			var item projectApp
			if err := rows.Scan(&item.projectID, &item.appID); err != nil {
				_ = rows.Close()
				return err
			}
		...
		if _, err := tx.ExecContext(ctx, `DELETE FROM project_app_bindings WHERE connection_id=?`, strings.TrimSpace(connectionID)); err != nil {
			return err
		}
		now := unixMS(time.Now())
		for _, item := range affected {
			if _, err := tx.ExecContext(ctx, `UPDATE project_app_bindings SET is_primary=1,updated_at=? WHERE id=(SELECT id FROM project_app_bindings WHERE project_id=? AND app_id=? ORDER BY created_at ASC LIMIT 1)`, now, item.projectID, item.appID); err != nil {
				return err
			}
		}
		return nil
...
func (s *Store) ResolveSessionAppConnection(ctx context.Context, sessionID, appID string) (string, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	var projectID string
	if err := s.db.QueryRowContext(ctx, `SELECT project_id FROM sessions WHERE id=?`, strings.TrimSpace(sessionID)).Scan(&projectID); errors.Is(err, sql.ErrNoRows) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT connection_id FROM project_app_bindings WHERE project_id=? AND app_id=? AND is_primary=1`, projectID, strings.TrimSpace(appID)).Scan(&connectionID)
...
func scanProjectAppBinding(row messageScanner) (*store.ProjectAppBinding, error) {
	var binding store.ProjectAppBinding
	var metadata string
	var created, updated int64
	if err := row.Scan(&binding.ID, &binding.ProjectID, &binding.AppID, &binding.ConnectionID, &binding.ResourceType, &binding.ResourceID, &binding.ResourceName, &metadata, &binding.Primary, &created, &updated); err != nil {
		return nil, err
	}
	if strings.TrimSpace(metadata) != "" {
		if err := json.Unmarshal([]byte(metadata), &binding.Metadata); err != nil {
			return nil, err
		}
	}
	binding.CreatedAt, binding.UpdatedAt = timeFromMS(created), timeFromMS(updated)
	return &binding, nil
}
