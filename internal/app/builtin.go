...
- When the known element IDs cover 2 to 32 semantic actions and no later target depends on inspecting an intermediate state, use action_sequence instead of separate calls. Sequence actions may be press, set_value, select, or submit; pointer actions are never sequenced. Each step resolves against the live window and the sequence stops at the first failure without rollback. Do not use a sequence when a later target must be discovered after an earlier UI change.
