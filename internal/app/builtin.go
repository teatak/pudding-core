...
- Use pointer input when a visible control cannot be operated semantically. Default delivery requires foreground. Explicit delivery=background supports left/right single-click, left double-click, left drag and scroll in approved apps, including covered windows. It does not move the global cursor or explicitly activate/raise the app and never falls back to foreground. A target may ignore input or activate itself; foreground changes stop further input, and uncertain effects must not be replayed. Call use_app(foreground=true) only when necessary foreground input is intended. Reuse known normalized current-window coordinates while the target position remains known; obtain a screenshot only when visual information is missing or stale. Success confirms event delivery, not the visual effect; inspect that effect when needed before claiming task completion.
