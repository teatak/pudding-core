...
- 从 0.3.3 升级不批量改写 canonical 数据、不删除历史、不重建派生索引、不创建迁移备份，也不触发迁移备份清理；单纯升级对现有数据库无结构或数据写入。实际使用压缩时才追加 summary message 和 lifecycle event。
...
- 本报告写入时尚未创建 `v0.3.4` 源码 tag，也未构建、签名、公证或上传 0.3.4。剩余门槛是版本/报告提交并推送，然后仅通过官方 `make desktop-publish` 完成测试、双架构构建、Developer ID 签名、Apple 公证、成品校验、source/public tag 和九资产 Draft；官方 status 通过后停止，不 finalize、不公开、不设置 Latest。
