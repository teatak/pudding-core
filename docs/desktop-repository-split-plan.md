...
已确认官网与 OAuth Worker 随 desktop 迁移，部署配置与 URL 保持不变。工程拆分时保留原 AGPL 声明；随后 core 改用 Apache-2.0 并公开。用户已确认 desktop 自有代码的版权或重新授权许可，desktop 从 `0.3.5` 起采用允许个人及商业用途免费使用的专有许可；第三方及历史授权保留。
