...
**现状**: 
- `ComposerTextArea` 通过 `useEffect` + `useRef` 做变更检测来通知父组件
- 这种模式容易出时序问题（如 Enter 不工作的 bug）

**建议**: 
考虑用 `useSyncExternalStore` 或 zustand store 来共享 `canSend`、`mentionMenuOpen` 等状态，避免回调 + state 的异步延迟问题。

### 4. 中文 IME 导致 textarea 高度变化破坏贴底

**问题链路**:
1. 中文 IME 输入时，候选词（英文拼音）内联显示在 textarea 中
2. 导致 textarea 内容换行，高度从 1 行增长到 2 行
3. Composer 整体变高 → 对话区域（transcript）的可视高度缩小
4. `TranscriptList` 的 `ResizeObserver` 检测到 viewport 缩小，但贴底逻辑可能没正确触发

**建议**: 
在 `onCompositionStart` 时锁定 textarea 高度（设置 `style.height` 为当前值），`onCompositionEnd` 时移除固定高度，恢复正常 auto-sizing。只改 `ComposerTextArea.tsx`。
...
#### 4. 每次新 turn 都要重新 `builtin_toolkit_load`

**问题**:
- `code.files-write` 每个 turn 只活一次
- 频繁重构时需要反复加载，增加了不必要的步骤

**建议**: 
考虑在 session 级别缓存 toolkit 加载状态，或者提供"记住此 toolkit"的选项。
...
**建议**: 
`code.lsp` 的 references 功能应该在重构场景中更主动地使用。
...
4. **状态同步模式优化**
   - 当前回调 + state 模式脆弱
   - 改用 `useSyncExternalStore` 或 zustand
   - 工作量：中等
...
本次任务完成了 8 个 UI 相关问题的修复，涉及侧边栏逻辑、diff 视图、输入性能、IME 兼容性等多个方面。

**日常小改动**（加个条件、改个样式）体验很流畅。但**中等规模重构**（拆组件、移函数）是痛苦区——patch 匹配不稳定 + 缺少引用分析 + 大 patch 容易失败，导致反复修修补补，效率下降明显。

建议优先解决 IME 兼容性和 textarea 高度锁定问题，这两个直接影响中文用户的核心体验。架构层面的改进可以逐步推进，不必一次性完成。
