...
更新：2026-09-09。状态：**隔离原型已实现，通用产品接入未通过验收**。

这是 [Codex 对比报告](computer-use-codex-comparison-2026-09-08.md) 之后的实施记录。没有更改正式 `PointerService`、LLM 工具 schema、权限或前台保护；未操作京东或修改业务数据。后续已追加真实镜像导航与隔离窗口的点击变量验证，结果见下方专项记录。

**最新进展（第九轮）：执行进程崩溃恢复、同 App 并发丢点击已在隔离原型修复；10 项生命周期、3 项并发/崩溃交叉测试及默认 Electron 单击回归通过。** 第八轮的真人并行 57 次点击样本保留，本轮未要求用户重复真人测试。仍未接入生产，不代表真实应用和全部输入动作已验收。前面各轮保留当时的失败证据，不应当作最新机制结论。
...
- 只使用 `postToPid`。输入发送阶段不切换系统前台、不置顶、不移动真实鼠标、不走全局 HID、不重放动作。第六轮另有显式的、仅限测试窗口的合成激活入口，会临时改变接收端内部 active/key 状态，结束后发送去激活通知；不能把这说成“目标内部完全没有焦点变化”。
...
公开 factory 改用窗口内坐标作为构造参数，也没有在本机修正跨进程接收端坐标。有效变体通过运行时解析的 `CGEventSetWindowLocation` 写入窗口局部位置；这是**非公开接口**，只留在隔离原型。滚轮另外先发送目标进程内的 mouseMoved，更新接收端窗口定位；不更新真实桌面鼠标。
...
1. 不依赖第三方应用开启首次点击，仍能正确执行普通单击/拖拽。普通单击已在两个受控接收端通过；第八轮通过一次真人打字和移动鼠标并行样本，合成激活下的拖拽、IME 及完整人工并行输入仍未验收，不能直接推广。
...
5. 通过上述门槛后才接入现有 `actions[]`、session/App 授权及结构化部分失败链路。批末可选观察、镜像文本输入优化仍未实施。

因此当前结论是：**后台输入已证明可行；显式窗口坐标加合成激活已解决受控接收端的普通单击问题。尚不是能替换现有前台指针后端的通用实现。**
