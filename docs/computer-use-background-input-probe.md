...
**最新进展（第十轮）：真实邮件 3 项、日历 4 项后台普通单击通过，包含目标点无遮挡和完全遮挡；飞书两项点击生效但随后成为系统前台，未通过无干扰验收。** 用户要求停止飞书后改测日历。第九轮的崩溃/并发修复和第八轮真人并行样本保留；仍未接入生产，不代表全部应用和输入动作已验收。前面各轮保留当时的失败证据，不应当作最新机制结论。
...
因此当前结论是：**后台普通单击已在受控接收端、邮件和日历指定场景通过；飞书仍会切到系统前台。尚不是能替换现有前台指针后端的通用实现。**
