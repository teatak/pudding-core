...
> 首版范围:Go 与 TypeScript / JavaScript 的语义 rename

## 1. 目标

C10 已让 agent 能通过统一 LSP 工具搜索符号、查找定义和引用、读取诊断。C11
进一步提供语义重命名,但不允许 language server 绕过 Pudding 的原子写入与审批边界。
...
它接收源文件位置与新名称,向对应 language server 请求 `WorkspaceEdit`,完整验证后转换为
Pudding 的原子 Patch 并在同一次工具调用中应用。Ask 模式先审批重命名操作,完成后通过
Turn 文件 Diff 审阅实际改动。
...
- 不把临时审批快照或 language server 状态写入 SQLite。
...
builtin_code_rename(path, line, column, new_name)
  ...
  -> convert to the atomic Patch representation
...
- 工具必须带 `sessionID`,用于 Project 授权、审批与 Turn 文件 Diff 归属。

成功输出复用 Patch Apply 结果字段,并增加 rename 元信息:
...
`prepareRename=null`、`rename=null` 或空 edit 都作为正常的结构化失败返回,不会执行写入。
...
- 超过原子 Patch 的 16 文件、单文件 512 KiB、总文本 2 MiB 或 diff
  256 KiB 限制。

不能过滤掉危险 edit 后继续应用部分结果。语义 rename 必须全有或全无,否则可能让
声明与引用不一致。

每个文件按当前磁盘内容重新读取,将协商后的 UTF-8 / UTF-16 / UTF-32 LSP position
严格换算为 byte offset。edit 先检查不重叠,再从文件尾向前应用到内存文本。最终把每个
文件的完整新文本交给原子 Patch 层,由其再次完成路径、类型、大小和内容校验。

## 7. 并发与一致性

- rename 请求前同步目标 document,确保光标文件与磁盘一致。
- language server 返回 edit 后读取所有目标文件并创建本次调用的写入快照。
- 从创建写入快照到事务提交期间,任何目标文件发生变化都以 `patch_stale` 拒绝,
  不覆盖外部修改。
- 多文件 apply 继续使用同目录临时文件、backup + rename 和逆序回滚。
- turn cancel 会取消正在等待的 LSP request;未完成转换时不写文件。

## 8. 审批语义

- `builtin_code_rename` 是 Project write risk;Auto 对 Project 内语义重命名自动放行。
  Ask 在调用 language server 前审批本次重命名操作,完成后的完整改动统一进入 Turn 文件 Diff。
- Full Access 可以跳过普通审批,但仍执行 Project path、源文件 hash 与事务校验。
...
详情复用 Patch 结果 card,顶部增加:

```text
oldName -> newName · Go / TypeScript · language server
```

下面继续显示文件数、增删行和文件列表;完整差异进入 Turn 文件 Diff。错误使用结构化原因与三语文案,不暴露
...
- `rename_too_large`:超出 patch 文件数、文件大小或 edit 数限制。

已有 language server crash、timeout、cancel、protocol、path 和 patch 错误继续复用。
...
| C11.1 | LSP capability、rename 请求、WorkspaceEdit 转换、原子 Patch 复用 | 已完成 |
...
- Ask 模式下工具调用后、审批通过前,工作树完全不变。
- 任一 edit 越权或不支持时整单失败,不产生部分写入。
- UTF-8 / UTF-16 / UTF-32 与 CRLF edit 坐标转换正确。
- 重叠 edit、资源操作、跨 root、符号链接和超限结果被稳定拒绝。
- apply 前文件漂移不会被覆盖,多文件失败不留下部分写入。
- transcript 显示 rename 元信息,完整改动可从 Turn 文件 Diff 打开,不暴露 snake_case。
- 真实 gopls 与 TypeScript server 均完成原子 rename 验证。
- `go test ./...`、Web build 与 `git diff --check` 通过。

## 13. 后续

rename 稳定后再评估 code action。Code action 仍只能接受可转换为相同原子 Patch
