...
> 产品边界:只支持 Electron 桌面端。停止移动端产品开发与响应式兼容,
> 新功能不增加移动端专用交互或验收项。
...
│ 新建会话             │ pane = header + 任务流 + composer │ 项目/画布/浏览器     │
│ 会话列表(带运行态)  │                                   │                     │
│ …                   │ ──────── 分屏分隔条 ────────      │                     │
...
- **运行模式识别**:壳加载 URL 附 `?shell=mac`,前端写入
  ...
  浏览器访问无此参数,零 inset。
- **业务 API 路线**:Electron shell 加载 URL 注入 daemon API base;前端业务
  REST/SSE/WS 直连 loopback daemon。Electron 只托管 UI/runtime,不反代业务 API。
- **全屏事件**:Electron main/preload 同步窗口状态,前端切换
  `data-fullscreen`;业务 API 仍走 daemon HTTP,desktop native 状态走 Electron IPC。
- **拖拽区**:rail 顶行与 pane header 的空白区设 `-webkit-app-region: drag`,
...
- 工作区是前端容器,包含项目、画布和浏览器;画布不再代表整个右栏。
- 项目与画布是固定入口,浏览器是 session-scoped 动态资源。
- 没有活动资源时显示工作区空态;进入空画布后不再显示浏览器的新建引导。
- 项目目录可通过 Electron 原生能力在系统终端中打开;产品不内嵌用户终端。Agent 的命令会话仍由工具运行时管理。
...
- 当前可见的项目文件、画布小组件或浏览器页只作为前端 UI 状态;用户发送消息时快照为中性的 `ui_context` content part,随 canonical user message 保存,不形成后端 focus 状态。
