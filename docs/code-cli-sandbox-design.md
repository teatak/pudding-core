...
| `auto` | 已授权项目内自主执行;已识别的风险或扩权需求仍确认 | 复杂脚本和普通命令使用同一项目沙箱;免审批不等于只读或绕过沙箱 |
| `full` | 不弹普通审批 | CLI 不套项目沙箱 |

默认的 host 访问是工具调用级授权:命令、参数、cwd 与 env 不得在批准后变更。需要确认但
不需要 host access 的删除等命令,批准后仍受项目沙箱保护。后台命令
也只在启动时使用该授权,不会把绕过权留给后续模型调用。

例外是用户明确选择的受限截图授权:直接、前台的 headless Chrome 命令,显式使用项目内
独立配置目录、项目内页面与 PNG 输出。审批卡可选择“本会话允许同类截图”;固定会话、
项目、roots、Chrome 路径及版本元数据、cwd、环境、页面、配置路径、输出目录及其余参数,
仅截图文件名和 `--window-size` 数值可变。新用途、其他页面/目录/参数、外部 URL、默认
浏览器资料、扩展加载、调试端口、复合 shell 均不能复用。普通 host 命令不提供此选项。
授权仅保存在当前 Engine 生命周期内,不落库;重启或释放会话资源后失效,UI 明示此边界。
一次性批准、拒绝与其他会话都不能产生该授权。

可执行文件预检复用沙箱的可读路径事实源,不再维护第二套系统安装目录名单。
因此 `/Applications/.../Chrome` 的执行本身不会要求额外项目目录授权;写入 App 安装目录
仍被拒绝。能读取/执行 App 不代表它的全部 macOS 服务都在沙箱中可用。
...
Auto 的授权单位是现有项目沙箱的执行范围，不是某种已证明只读的语法。
循环（包括动态列表/嵌套循环）、条件、函数、变量、命令替换、动态重定向、Shell 和
Python/Node 脚本不再仅因无法静态分析而审批；删除旧的只读循环展开和特殊语法豁免路径。
AST 仅提取可识别的命令、路径、环境和重定向，保留未知参数的位置，继续检查后续明确风险。
字面量 Shell/eval 内容有界检查；不求值变量、不运行探测命令、不拆分执行。
已识别的危险操作、越界路径或 host 请求仍按整条调用处理。

无法预判的文件访问由 OS 沙箱在实际访问时限制，子进程继承同一限制；不会自动扩展
项目目录、切换 Full 或使用 host 重跑。动态失败前可能已有部分副作用，禁止自动整段重试。
Ask/Full 语义、原始执行文本、严格的运行验证与会话授权复用条件不变。
`LowRisk` 对 command 类表示可以在 Auto 的既有沙箱内启动，不表示只读或完整安全证明。

风险接受范围：项目内可写，代码可能破坏项目；当前文件快照有大小/覆盖限制，不保证全量
恢复。网络仍允许出站，不能保证任意脚本不会外发项目数据；本次没有引入受控网络、凭据
代理或隔离工作副本。不以命令黑名单代替这些系统级隔离能力。

Git `ls-tree` 纳入低风险查询；AWK 首个 action（`{...}`）之前的 `>`/`>=` 筛选比较
不再误判为输出重定向。action 内的 `>` 仍保守确认，AWK 的 `@include`/`@load`、
`system` 和管道仍需确认。
