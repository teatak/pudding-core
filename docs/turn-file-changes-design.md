...
> 2026-09-19 更新：下文全项目快照生命周期属于第一版历史设计，已被显式目标追踪替代。当前以工具调用前后快照累计本轮净变化，完成时直接持久化，不重新扫描工作区。前台命令除显式输出目标外，也观察本轮已经追踪的具体文件：脚本自删或通过不透明命令删除后，新增再删除的文件不再出现在最终卡片。原有文件被删除仍记录为删除；恢复原文则无净变化。不透明命令不发现无关文件，已追踪文件变成目录也不递归扫描；后台命令仍不纳入调用边界追踪。历史卡片不随当前文件系统变化被反向过滤。
