...
test("translates and interpolates native menu labels", () => {
  assert.equal(nativeText("zh-CN", "appName"), "Pudding");
  assert.equal(nativeText("zh-CN", "openApp", { app: nativeText("zh-CN", "appName") }), "打开 Pudding");
  assert.equal(nativeText("en", "openApp", { app: "Pudding" }), "Open Pudding");
  assert.equal(nativeText("zh-TW", "quitApp", { app: nativeText("zh-TW", "appName") }), "結束 Pudding");
